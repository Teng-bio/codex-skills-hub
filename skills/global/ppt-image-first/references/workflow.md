...
Final PPTX export compatibility:
- prefer a standards-compliant PPTX writer such as `python-pptx` for final delivery
- avoid hand-written minimal OOXML packages as the default final export path, because WPS may reject them even when the page images are valid
- for image-first decks, create blank `16:9` slides and insert each approved full-slide image as the slide content unless approved editable overlays are required
- if the active Python does not have `python-pptx`, search known local Python / conda environments before installing dependencies; ask the user before installing anything new
- when creating a compatibility rebuild, name it clearly, e.g. with `_WPS_compatible.pptx`
- validate before handoff where possible: run `unzip -t`; if LibreOffice / `soffice` is available, run a headless conversion or open probe
- if compatibility validation fails, regenerate with a standards-compliant PPTX writer before delivering the file
